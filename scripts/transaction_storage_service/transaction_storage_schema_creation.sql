...
CREATE USER IF NOT EXISTS `transactionstorageadmin`@`%` IDENTIFIED WITH mysql_native_password BY 'password';
...
CREATE USER IF NOT EXISTS `transactionstorageapp`@`%` IDENTIFIED WITH mysql_native_password BY 'password';
