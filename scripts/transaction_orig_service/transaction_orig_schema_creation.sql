...
CREATE USER IF NOT EXISTS `transactionorigadmin`@`%` IDENTIFIED WITH mysql_native_password BY 'password';
...
CREATE USER IF NOT EXISTS `transactionorigapp`@`%` IDENTIFIED WITH mysql_native_password BY 'password';
